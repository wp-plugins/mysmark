  ...
  protected function getSessionObject($access_token = NULL) {
    ...
    if (!empty($access_token) && isset($access_token['access_token'])) {
      $session = array(
        ...
        'expirse'       => isset($access_token['expires_in']) ? time() + $access_token['expires_in'] : time() + $this->getVariable('expires_in', OAUTH2_DEFAULT_EXPIRES_IN),
        'refresh_token' => isset($access_token['refresh_token']) ? $access_token['refresh_token'] : '',
        'scope'         => isset($access_token['scope']) ? $access_token['scope'] : '',
        'secret'        => md5(base64_encode(pack('N6', mt_rand(), mt_rand(), mt_rand(), mt_rand(), mt_rand(), uniqid()))) 
      );
  ...
  protected function makeRequest($path, $method = 'GET', $params = array(), $ch = NULL) {
    ...
    if (strpos($headers[0], 'HTTP/1.1 4') !== FALSE || strpos($headers[0], 'HTTP/1.1 5') !== FALSE) {
      ...
      if (preg_match('/^HTTP\/1.1 ([0-9]{3,3}) (.*)$/', $headers[0], $matches)) {
        ...
        $result['response'] = json_decode( $body );
      }
